...
- **Transport pressure (P0)**: Route AutoCAD MCP calls through one FIFO lane with a bounded queue and sliding-window budget. Never launch parallel mutation or screenshot calls; on a retryable admission error, stop the round and preserve its evidence.
- **Response truth (P1)**: Screenshot feedback returns a managed path, dimensions, byte count, and SHA-256 by default. Inline base64 requires explicit opt-in and a size ceiling; large audit/entity responses must be narrowed or read from saved artifacts.
